...
# ============================================================
# 4. READ DOCUMENTS (Using RecursiveCharacterTextSplitter)
# ============================================================
...
def load_documents():
    ...
    if not files:
        raise FileNotFoundError("No .txt files found inside documents/")
    ...
    for file_path in files:
        ...
        # Split using the recursive splitter
    ...
    print(f"Loaded {len(chunks)} chunks from {len(files)} documents using RecursiveCharacterTextSplitter.")
...
def build_faiss():
    ...
    print(f"FAISS created with {len(chunks)} chunks.")
    return index


# ============================================================
# 6. CACHE MANAGEMENT (MD5/SHA256 & TTL)
# ============================================================
...
def check_cache(question):
    cache = load_cache()
    # Create SHA256 hash of the question
    ...
    if q_hash in cache:
        ...
        if current_time - entry["timestamp"] <= CACHE_TTL:
            print(f"\n⚡ [CACHE HIT] Found answer! (Hash: {q_hash[:8]}...)")
            return entry["answer"]
        else:
            print("\n⏳ [CACHE EXPIRED] 24-hour TTL exceeded. Regenerating answer...")
            del cache[q_hash]
            save_cache(cache)
    return None
...
def add_to_cache(question, answer):
    ...
    save_cache(cache)
    print(f"\n💾 Answer saved to cache with 24h TTL (Hash: {q_hash[:8]}...)")


# ============================================================
# 7. RETRIEVAL & CONFLICT DETECTION
# ============================================================

def search_documents(question, index, k=5):
...
def sangamrag(question, index):
    ...
    if cached_answer:
        print("\n" + "-" * 70)
        print("MODE: CACHE HIT")
        print("-" * 70)
        print(cached_answer)
        return {"answer": cached_answer, "mode": "cache", "sources": []}

    print("\n[1] Retrieving relevant documents...")
    results = search_documents(question, index, k=5)
    ...
    if conflicts:
        ...
        return {"answer": answer, "mode": "conflict", "sources": results}

    print("✓ No obvious numeric conflict detected.")
    ...
    return {"answer": answer, "mode": mode, "sources": results}
